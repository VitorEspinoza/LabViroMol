...
To run the whole stack the way it runs in production — API + Angular admin panel + Next.js institutional site behind an nginx gateway, all built from local source — clone [`LabViroMol-Admin-Panel`](https://github.com/VitorEspinoza/LabViroMol-Admin-Panel) and [`labviromol-institucional`](https://github.com/rafaelleonardodev/labviromol-institucional) as siblings of this repo and run `docker compose up -d --build`; `docker-compose.override.yml` picks up the frontends automatically. See [Running the full stack locally](./docs/getting-started.md#running-the-full-stack-locally-docker-compose) for the full steps, including how `.env` is filled in without an existing database to connect to.
