...
{
    Task<Result<(Guid UserId, string ResetLink)>> CreateUserAsync(string email, CancellationToken ct);
    ...
    Task<Result<(string ResetLink, string FirstName)>> GeneratePasswordResetTokenAsync(string email, CancellationToken ct);
